...
class PhaseTransitionManager:
    ...
    def _is_phase_complete(self, parsed_results: Dict) -> bool:
        ...
        if self.current_phase == ModelPhase.DIAGNOSE_STRUCTURE:
            # Complete when: Spent at least 2 iterations checking structure
            # (Max 5 iterations enforced by forced transition)
            if self.iterations_in_phase >= 2:
                return True
            return False
        ...
        if self.current_phase == ModelPhase.OPTIMIZE_IIV:
            # Complete when:
            # - Shrinkage is good (<50%) + OFV stable → Ready for covariates
            # Note: If goal NOT achieved, forced transition (5 iterations) handles it
